...
export default function UserPlaylist({ details }) {
  ...
  const DeletePlaylist = async (k) => {
    try {
      const token = SessionRetrive("accessToken");
      if (!token) throw Error("Invalid token");
      const res = await DeleteMedia(data[k].id, token);
      if (res.status == "ok") {
...
function PlaylistSong({ id, playlist_id, uid }) {
  ...
  const RemoveSong = async (playlist_id, track_id) => {
    try {
      const token = SessionRetrive("accessToken");
      if (!token) throw Error("Invalid token");
      const res = await RemoveTrackPlaylist(playlist_id, track_id, token);
      if (res.status == "ok") {
    ...
    <div className="mx-auto flex flex-row justify-between rounded h-16 w-56  bg-neutral-800 p-2">
      <div className="flex flex-row w-full justify-between gap-4">
        ...
        <div className="flex flex-row w-full justify-between">
          ...
          <button
            className="p-2 object-contain w-8"
            onClick={(e) => {
              e.stopPropagation();
              RemoveSong(playlist_id, id);
            }}
          >
